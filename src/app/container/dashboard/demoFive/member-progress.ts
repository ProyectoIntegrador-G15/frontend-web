  ...
  <div class="px-[25px] pt-0 pb-[25px] min-h-[345px]">
    <div *ngFor="let tab of tabData" [class.hidden]="sellingTab !== tab.key">
      <div class="overflow-x-auto w-full">
        <nz-table [nzData]="memberProgressData[tab.key]" [nzFrontPagination]="false" [nzShowPagination]="false"
            ...
            <tr *ngFor="let product of memberProgressData[tab.key]" class="group">
              <td
                ...
                <div class="flex items-center gap-[10px]">
                  <div
                    class="overflow-hidden flex justify-center relative w-[32px] h-[32px] rounded-[8px] bg-{{product.progress}}/10">
                    <img class="absolute bottom-0" src="assets/images/progress/{{product.userImg}}"
                      [alt]="product.userImg" />
                  </div>
                  <span
                    class="font-medium capitalize text-dark dark:text-white/[.87] text-15">{{ product.title }}</span>
                </div>
              </td>
              <td
              ...
              <td
                ...
                <div class="d-flex align-center justify-content-end min-w-[11.25rem]">
                  <nz-progress class="member-progress text-{{product.progress}} group/item text-{{product.progress}}"
                    [nzPercent]="product.status"></nz-progress>
                </div>
              </td>
            </tr>
          </tbody>
