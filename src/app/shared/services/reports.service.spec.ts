...
describe('ReportsService', () => {
  ...
  describe('getReportDownloadUrl', () => {
    it('should get download URL successfully', () => {
      ...
      service.getReportDownloadUrl(1).subscribe((response) => {
        expect(response).toBeTruthy();
        expect(response.download_url).toBe(mockDownloadResponse.download_url);
        expect(response.report_id).toBe(1);
        expect(response.expires_in_minutes).toBe(60);
      });

      const req = httpMock.expectOne(`${baseUrl}/reports/1/download`);
      expect(req.request.method).toBe('GET');
      req.flush(mockDownloadResponse);
    });

    it('should handle error when getting download URL fails', (done) => {
      const errorMessage = 'Reporte no encontrado';

      service.getReportDownloadUrl(999).subscribe({
        next: () => {
          done.fail('Should have failed');
        },
        error: (error) => {
      ...
      // ApiService tiene retry(2), así que hay 1 petición inicial + 2 reintentos = 3 peticiones
      const req1 = httpMock.expectOne((request) => 
        request.url.includes('/reports/999/download') && request.method === 'GET'
      );
      req1.flush({ message: errorMessage }, { status: 404, statusText: 'Not Found' });

      const req2 = httpMock.expectOne((request) => 
        request.url.includes('/reports/999/download') && request.method === 'GET'
      );
      req2.flush({ message: errorMessage }, { status: 404, statusText: 'Not Found' });

      const req3 = httpMock.expectOne((request) => 
        request.url.includes('/reports/999/download') && request.method === 'GET'
      );
      req3.flush({ message: errorMessage }, { status: 404, statusText: 'Not Found' });
    });

    it('should handle network error', (done) => {
      service.getReportDownloadUrl(1).subscribe({
        next: () => {
          done.fail('Should have failed');
        },
        error: (error) => {
      ...
      // ApiService tiene retry(2), así que hay 1 petición inicial + 2 reintentos = 3 peticiones
      const req1 = httpMock.expectOne((request) => 
        request.url.includes('/reports/1/download') && request.method === 'GET'
      );
